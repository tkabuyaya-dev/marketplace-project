...
import { Product, User, SubscriptionTier, Category, Country, Marketplace, Currency, SubscriptionRequest } from '../types';
import {
...
export const AdminDashboard: React.FC = () => {
  ...
  const loadSubRequests = async () => {
    const requests = await getAllSubscriptionRequests(
      subRequestFilter === 'all' ? undefined : subRequestFilter as any
    );
    setSubRequests(requests);
  };

  useEffect(() => {
    if (activeTab === 'subs') loadSubRequests();
  }, [activeTab, subRequestFilter]);

  const handleApproveRequest = async (requestId: string) => {
    try {
      await approveSubscriptionRequest(requestId, currentUser.id);
      toast('Abonnement active avec succes !', 'success');
      loadSubRequests();
      refreshData();
    } catch (err) {
  ...
  const pendingRequestsCount = subRequests.filter(r => r.status === 'pending' || r.status === 'pending_validation').length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-2 border-b border-gray-800 pb-3">
        <button onClick={() => setSubSubTab('requests')}
          ...
          Demandes {pendingRequestsCount > 0 && <span className="ml-1 bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{pendingRequestsCount}</span>}
        </button>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {['all', 'pending', 'pending_validation', 'approved', 'rejected'].map(f => (
              <button key={f} onClick={() => setSubRequestFilter(f)}
                ...
                {f === 'all' ? 'Toutes' : f === 'pending' ? 'En attente' : f === 'pending_validation' ? 'Paiement soumis' : f === 'approved' ? 'Approuvees' : 'Rejetees'}
              </button>
            ))}
          </div>
          ...
          {subRequests.length === 0 ? (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center">
              <p className="text-gray-500">Aucune demande {subRequestFilter !== 'all' ? 'avec ce filtre' : ''}</p>
            </div>
          ) : (
            <div className="space-y-3">
              {subRequests.map(req => (
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                          ...
                          {req.status === 'pending' ? 'En attente' : req.status === 'pending_validation' ? 'Paiement soumis' : req.status === 'approved' ? 'Approuve' : 'Rejete'}
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-3 text-xs text-gray-400">
                        <span>Plan: <strong className="text-gold-400">{req.planLabel}</strong></span>
                        <span>Montant: <strong className="text-white">{req.amount.toLocaleString()} {req.currency}</strong></span>
                        <span>Max: <strong className="text-white">{req.maxProducts >= 99999 ? 'Illimite' : req.maxProducts}</strong></span>
                        <span>{formatDate(req.createdAt)}</span>
                      </div>
                      {req.transactionRef && (
                        <p className="text-xs text-blue-400 mt-1">Ref: <span className="font-mono bg-blue-500/10 px-1.5 py-0.5 rounded">{req.transactionRef}</span></p>
                      )}
                      {req.rejectionReason && (
                        <p className="text-xs text-red-400 mt-1">Raison: {req.rejectionReason}</p>
                      )}
                    </div>
          ...
          {rejectingRequestId && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
              <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md mx-4">
                <h3 className="text-white font-bold mb-3">Raison du rejet</h3>
                <textarea
                  value={rejectRequestReason}
                  onChange={e => setRejectRequestReason(e.target.value)}
                  placeholder="Ex: Reference de paiement invalide..."
      ...
      {subSubTab === 'tiers' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-white">Configuration des Paliers</h2>

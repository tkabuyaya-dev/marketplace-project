...
export const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose, onProductClick }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-gray-950/95 backdrop-blur-2xl animate-fade-in flex flex-col font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex-1 group">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                {query && (
                  <button onClick={() => setQuery('')} aria-label={t('search.clear')} className="min-w-[44px] min-h-[44px] flex items-center justify-center text-gray-500 hover:text-white rounded-full hover:bg-gray-700">
              ...
              {showSuggestions && (suggestions.length > 0 || autoProducts.length > 0) && (
                <div
                  ...
                  className="absolute top-full left-0 right-0 mt-2 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden z-30 max-h-[70vh] overflow-y-auto"
                  role="listbox"
                >
                      ...
                      {autoProducts
                        ...
                        .map((p) => (
                          <button
                            ...
                            className="w-full text-left px-4 min-h-[44px] py-3 text-sm text-gray-300 hover:bg-gray-800 hover:text-white flex items-center gap-3 transition-colors"
                            role="option"
                            aria-selected={false}
                          >
                            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-gray-600 shrink-0"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                            <span className="truncate flex-1 text-left">{highlightMatch(p.title, query)}</span>
                            <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className="text-gray-700 shrink-0 -rotate-45"><path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14M12 5l7 7-7 7" /></svg>
                          </button>
                        ))}
                    </>
                  )}
            ...
            <button
              ...
              className="p-3.5 rounded-2xl border bg-blue-600 text-white border-blue-500 hover:bg-blue-500 transition-all flex items-center"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recentSearches.map((term, i) => (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800/70 border border-gray-700/50 rounded-full text-sm text-gray-300 hover:text-white hover:border-gray-600 transition-all"
                      >
                        <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-gray-600">
                    ...
                    {discoverProducts.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex items-center gap-2.5 bg-gray-800/60 border border-gray-700/40 rounded-xl p-2.5 hover:bg-gray-800 hover:border-gray-600 transition-all text-left"
                        >
                          {/* Thumbnail */}
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-white font-medium leading-snug line-clamp-2 mb-1">
                              {p.title}
                            </p>
                            {discountPct && discountPct > 0 && (
                              <p className="text-[10px] text-red-400 font-semibold leading-none mb-0.5">
                                Sale -{discountPct}% now
                              </p>
                            )}
                            <p className="text-[11px] text-amber-400 font-bold leading-none">
                              {displayPrice.toLocaleString('fr-FR')} <span className="text-[9px] font-normal text-amber-400/70">{cur}</span>
                            </p>
                          </div>
                        </button>
                      );
      ...
      <div className="hidden md:flex items-center justify-center gap-4 py-2.5 text-[10px] text-gray-600 border-t border-gray-800 bg-gray-900">
        <span><kbd className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 font-mono">↑</kbd> <kbd className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 font-mono">↓</kbd> {t('search.kbNavigate')}</span>
        <span><kbd className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 font-mono">↵</kbd> {t('search.kbSelect')}</span>
        <span><kbd className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 font-mono">Esc</kbd> {t('search.escToClose')}</span>
      </div>
    </div>
  );
};
